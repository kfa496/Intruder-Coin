...
contract ('Intruder',function(accounts)  {
	var tokenInstance;
	it('intializes the contract with correct value',function(){
		return Intruder.deployed().then(function(instance){
			tokenInstance = instance;
			return tokenInstance.name();
		}).then(function(name){
			assert.equal(name,'Intruder Token','has the correct name');
			return tokenInstance.symbol();
		}).then(function(symbol){
			assert.equal(symbol,'IGX','has the correct symbol');
			return tokenInstance.standard();
		}).then(function(standard){
			assert.equal(standard,'Intruder Token v1.0','has the correct symbol');
